...
var lunarstoneHistoryNameArray = []
...
class HistoryArrayGroup extends React.Component {
	...
	async SyncFileGroupServer(){
		var ret = new Array()
		var list = new Array("lunarstone","eightrandom","taiyi","sixcourse","qimen","Partnership","Marry")
		var ret = new Array()
		...
		ret =(async function(ret){
			var ret = new Array()
			await StorageModule.getAllDataForKey("lunarstone").then(ids => {
				for (i = 0; i < ids.length; i++) {
					try {
						console.log("SyncFileGroupServer",ids[i])
						var Jobj = JSON.parse(ids[i]);
	...
	async GetlunarstoneHistory() {
		await this.ReloadlunarstoneHistory()
		//console.log("GetlunarstoneHistory", lunarstoneHistoryNameArray[0])
		return lunarstoneHistoryNameArray;
	...
	async SyncAllHistory(){
		...
		await this.GetlunarstoneHistory()
				...
				try {
					...
					if(ids[i].constructor===Array ){
						...
						QimenHistoryNameArray[i] = obj
												/*
						这里根据数据升级格式，需要和lunarstonenewpage的生产数据做比较，统一格式
						如果格式不统一，则出现重要问题
						*/
						var Jobj = {}
						Jobj.id = savedate[0]
	...
	async ReloadlunarstoneHistory() {
		lunarstoneHistoryNameArray = []
		await StorageModule.getAllDataForKey("lunarstone").then(async(ids) => {
			for (i = 0; i < ids.length; i++) {
				try {
					var Jobj = JSON.parse(ids[i]);
					var result = SixrandomModule.get_lunarstone_name(Jobj.lunar)
					var timedate = new Date(Number(Jobj.date))
					var obj = {
						name: "求测：" + Jobj.question,
						ret: SixrandomModule.get_lunarstone_name(Jobj.lunar),
						tip: Jobj.tip,
						title: SixrandomModule.get_lunarstone_name(Jobj.lunar),
						time: Jobj.id,
						star: Jobj.star,
						url: "?date=" + timedate + "&lunar=" + Jobj.lunar + "&question=" + Jobj.question + "&kind=lunarstone" + "&rowid=" + Jobj.id ,
						id: Jobj.date,
					}
					lunarstoneHistoryNameArray[i] = obj
					/*
					这里根据数据升级格式，需要和lunarstonenewpage的生产数据做比较，统一格式
					如果格式不统一，则出现重要问题
					*/
					Jobj.id = Jobj.date
					Jobj.kind = "lunarstone"
					Jobj.sync = undefined==Jobj.sync ? false:Jobj.sync
					Jstr = JSON.stringify(Jobj)
					let T = await this.loadid(Jobj.kind,Jobj.id)
					if (undefined!=T) {
					...
					lunarstoneHistoryNameArray[i] = undefined
				}
			}
			lunarstoneHistoryNameArray.reverse()
			for (var i = 0, len = lunarstoneHistoryNameArray.length; i < len; i++) {
				if (undefined == lunarstoneHistoryNameArray[i]) {
					lunarstoneHistoryNameArray.splice(i, 1);
					len--;
					i--;
				}
			}
			//console.log("ReloadlunarstoneHistory", lunarstoneHistoryNameArray[0])
		});
	}
			...
			for (i = 0; i < ids.length; i++) {
				try {
						...
						*/
						...
						//这里逻辑和lunarstone不同，是从name里读数据装入eightrandom然后服务器同步以后，返回删除name
						if (undefined==T) {
