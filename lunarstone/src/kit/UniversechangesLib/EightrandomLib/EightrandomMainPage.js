...
class EightrandomMainPage extends React.Component {
  ...
  async deletethis() {
    ...
    HistoryArrayGroup.loadid('eightrandom', rowid).then(async ret => {
      ...
      //this.props.navigation.navigate("lunarstoneHistoryPage",{ text: "refresh" })
    });
  }
  ...
  refreshlist() {
    ...
    if (undefined != parameter) {
      ...
      var beginlucky = EightrandomModule.getbigluckyearbegin(
        retterm,
        gz,
        info.EightDate,
        info.sex,
      ...
      console.log(
        'beginlucky',
        Math.floor(beginlucky),
        Number(gz.getFullYear()),
      );
  ...
  buildeight(sex) {
    var buildeight = new Array();
    buildeight[0] = EightrandomModule.parentday(
      this.state.EightDate[0],
      this.state.EightDate[4],
    );
    buildeight[2] = EightrandomModule.parentday(
      this.state.EightDate[2],
      this.state.EightDate[4],
    );
    buildeight[4] = '乾造' == sex ? '元男' : '元女';
    buildeight[6] = EightrandomModule.parentday(
      this.state.EightDate[6],
      this.state.EightDate[4],
    );
    buildeight[1] = EightrandomModule.parentearth(
      this.state.EightDate[1],
      this.state.EightDate[4],
    );
    buildeight[3] = EightrandomModule.parentearth(
      this.state.EightDate[3],
      this.state.EightDate[4],
    );
    buildeight[5] = EightrandomModule.parentearth(
      this.state.EightDate[5],
      this.state.EightDate[4],
    );
    buildeight[7] = EightrandomModule.parentearth(
      this.state.EightDate[7],
      this.state.EightDate[4],
    );
    var buildeightExt = new Array();
    ...
    buildeightExt[1] = EightrandomModule.gethideshishen(
      buildeightExt[0],
      this.state.EightDate[4],
    );
    buildeightExt[3] = EightrandomModule.gethideshishen(
      buildeightExt[2],
      this.state.EightDate[4],
    );
    buildeightExt[5] = EightrandomModule.gethideshishen(
      buildeightExt[4],
      this.state.EightDate[4],
    );
    buildeightExt[7] = EightrandomModule.gethideshishen(
      buildeightExt[6],
      this.state.EightDate[4],
    );
    var precent = new Array();
    ...
    luckyyear = EightrandomModule.getbigluckyear(
      this.state.EightDate,
      this.state.sex,
    );
    ...
    for (var i in luckyyear) {
      ...
      luckyyearposition.push(
        EightrandomModule.gettwelfthposition(
          this.state.EightDate[4] + luckyyear[i].slice(1, 2),
        ),
      );
    }
  ...
  getColor(king, size) {
    if ('甲' == king || '乙' == king || '寅' == king || '卯' == king) {
      return (
        <Text
          style={[styles.Eightstylewithfont, {color: 'green', fontSize: size}]}>
          {king}
        </Text>
      );
    }
    if ('丙' == king || '丁' == king || '午' == king || '巳' == king) {
      return (
        <Text
          style={[styles.Eightstylewithfont, {color: 'red', fontSize: size}]}>
          {king}
        </Text>
      );
    }
    if (
      ...
      return (
        <Text
          style={[
            styles.Eightstylewithfont,
            {color: '#8B4513', fontSize: size},
          ]}>
          {king}
        </Text>
      );
    ...
    return (
      <Text style={([styles.Eightstylewithfont], {fontSize: size})}>
        {king}
      </Text>
    );
  }
  ...
  renderItem(item) {
    return (
      <Text key={item.item} style={styles.flatTextfone}>
        {item.item}
      </Text>
    );
  }
  ...
  changeyear(bigyear, miniyear) {
    ...
    if ('' !== bigyear) {
      ...
      my = Number(miniyear);
      if (my - 70 > this.state.beginlucky && my - 70 > 1900) {
        my = my - 60;
      }
      if (my > 2100) {
        my = 2100;
    ...
    if (undefined != hide) {
      return (
        <View style={styles.gridfix}>
          <Text style={{fontSize: FontStyleConfig.getFontApplySize() + 14}}>
            {hide}
          </Text>
        </View>
      );
  ...
  createpie(display, map) {
    var indexcolor = '子丑寅卯辰巳午未申酉戌亥'.indexOf(
      this.state.EightDate[3],
    );
    var colorsel = [
      '#1E90FF',
    ...
    if (this.state.precent != '') {
      var datafive = [
        {
          x: 1,
          y:
            '空' != this.state.enhance
              ? map[this.state.enhance][0] * this.state.precent[5]
              : this.state.precent[5],
          label: '木',
        },
        {
          x: 2,
          y:
            '空' != this.state.enhance
              ? map[this.state.enhance][2] * this.state.precent[6]
              : this.state.precent[6],
          label: '火',
        },
        {
          x: 3,
          y:
            '空' != this.state.enhance
              ? map[this.state.enhance][4] * this.state.precent[7]
              : this.state.precent[7],
          label: '土',
        },
        {
          x: 4,
          y:
            '空' != this.state.enhance
              ? map[this.state.enhance][5] * this.state.precent[8]
              : this.state.precent[8],
          label: '金',
        },
        {
          x: 5,
          y:
            '空' != this.state.enhance
              ? map[this.state.enhance][8] * this.state.precent[9]
              : this.state.precent[9],
          label: '水',
        },
      ];
      ...
      returncontent = (
        <View style={[{textAlign: 'center', alignItems: 'center'}]}>
          <SegmentedControl
            ...
            onChange={e =>
              this.setState(
                {enhance: e.nativeEvent.value},
                console.log('selvales', e.nativeEvent.value),
              )
            }
          />
      ...
      console.log('createpie', returncontent);
  ...
  render() {
    ...
    var r = EightrandomModule.getrelationship(
      this.state.EightDate,
      gzYear[1],
      curluckyear,
      this.state.sex,
    ...
    minluckyyear = EightrandomModule.getminlucky(
      birthdayyear,
      this.state.sex,
      curyear,
    );
    ...
    test.push({
      info: EightrandomModule.parentday(
        curluckyear[0],
        this.state.EightDate[4],
      ),
      hide: '',
    ...
    test.push({
      info: EightrandomModule.parentday(gzYear[0], this.state.EightDate[4]),
      hide: '',
    });
    for (var i = 0; i < 4; i++) {
    ...
    test.push({info: '十神', hide: ''});
    test.push({
      info: EightrandomModule.parentearth(
        curluckyear[1],
        this.state.EightDate[4],
      ),
      hide: '',
    ...
    test.push({
      info: EightrandomModule.parentearth(gzYear[1], this.state.EightDate[4]),
      hide: '',
    });
    ...
    hidelist.forEach(element => {
      hindinfo.push(
        element + EightrandomModule.parentday(element, this.state.EightDate[4]),
      );
    });
    ...
    for (var i = 0; i < 4; i++) {
      ...
      hidelist.forEach(element => {
        hindinfo.push(
          element +
            EightrandomModule.parentday(element, this.state.EightDate[4]),
        );
      });
      test1.push({info: hindinfo, hide: ''});
    }
    ...
    */
    ...
    test2.push({
      info: EightrandomModule.gettwelfthposition(
        this.state.EightDate[4] + curluckyear[1],
      ),
      hide: '',
    });
    test2.push({
      info: EightrandomModule.gettwelfthposition(
        this.state.EightDate[4] + gzYear[1],
      ),
      hide: '',
    });
    for (var i = 0; i < 4; i++) {
      var x = EightrandomModule.gettwelfthposition(
        this.state.EightDate[4] + this.state.EightDate[i * 2 + 1],
      );
    ...
    test2.push({
      info: EightrandomModule.gettwelfth(curluckyear[0] + curluckyear[1]),
      hide: '',
    });
    test2.push({
      info: EightrandomModule.gettwelfth(gzYear[0] + gzYear[1]),
      hide: '',
    });
    for (var i = 0; i < 4; i++) {
      var x = EightrandomModule.gettwelfth(
        this.state.EightDate[i * 2] + this.state.EightDate[i * 2 + 1],
      );
      test2.push({info: x, hide: ''});
    }
    ...
    for (var i = 0; i < 8; i++) {
      yearsnumber.push(
        i == 0 ? this.state.beginlucky : yearsnumber[i - 1] + 10,
      );
    }
    var years = new Array();
    years = luckyearrelation.concat(
      yearsnumber,
      this.state.luckyyear,
      luckyyearposition,
    );
    ...
    display['甲'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][0] * this.state.daykey['甲'])
        : this.state.daykey['甲'];
    display['乙'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][1] * this.state.daykey['乙'])
        : this.state.daykey['乙'];
    display['丙'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][2] * this.state.daykey['丙'])
        : this.state.daykey['丙'];
    display['丁'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][3] * this.state.daykey['丁'])
        : this.state.daykey['丁'];
    display['戊'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][4] * this.state.daykey['戊'])
        : this.state.daykey['戊'];
    display['己'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][5] * this.state.daykey['己'])
        : this.state.daykey['己'];
    display['庚'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][6] * this.state.daykey['庚'])
        : this.state.daykey['庚'];
    display['辛'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][7] * this.state.daykey['辛'])
        : this.state.daykey['辛'];
    display['壬'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][8] * this.state.daykey['壬'])
        : this.state.daykey['壬'];
    display['癸'] =
      '空' != this.state.enhance
        ? Math.floor(map[this.state.enhance][9] * this.state.daykey['癸'])
        : this.state.daykey['癸'];
    var totalfive =
      display['甲'] +
      display['乙'] +
    ...
    fiveprecent['木'] = Math.floor(
      ((display['甲'] + display['乙']) * 100) / totalfive,
    );
    fiveprecent['火'] = Math.floor(
      ((display['丙'] + display['丁']) * 100) / totalfive,
    );
    fiveprecent['土'] = Math.floor(
      ((display['戊'] + display['己']) * 100) / totalfive,
    );
    fiveprecent['金'] = Math.floor(
      ((display['庚'] + display['辛']) * 100) / totalfive,
    );
    fiveprecent['水'] = Math.floor(
      ((display['壬'] + display['癸']) * 100) / totalfive,
    );
    ...
    var day = EightrandomModule.getselfinfo(
      this.state.EightDate[4] + this.state.EightDate[5],
    );
    ...
    for (i = 0; i < 4; i++) {
      ...
      shensha[i] =
        shensha[i] +
        EightrandomModule.shensha_dayg2earthz(
          this.state.EightDate[4],
          this.state.EightDate[i * 2 + 1],
        );
      shensha[i] =
        shensha[i] +
        EightrandomModule.shensha_moon(
          this.state.EightDate[3],
          this.state.EightDate[i * 2],
        );
      shensha[i] =
        shensha[i] +
        EightrandomModule.shensha_yearg2earthz(
          this.state.EightDate[0],
          this.state.EightDate[i * 2 + 1],
        );
      if (i != 1) {
      ...
      if (i != 2) {
        shensha[i] =
          shensha[i] +
          EightrandomModule.shensha_dayz2earthz(
            this.state.EightDate[5],
            this.state.EightDate[i * 2 + 1],
          ); //日支不见自己
      }
      if (i != 0) {
        shensha[i] =
          shensha[i] +
          EightrandomModule.shensha_tianluo(
            this.state.EightDate[0] + this.state.EightDate[1],
            this.state.EightDate[i * 2 + 1],
          ); //年支不见年支
        shensha[i] =
          shensha[i] +
          EightrandomModule.shensha_diwang(
            this.state.EightDate[0] + this.state.EightDate[1],
            this.state.EightDate[i * 2 + 1],
          ); //年支不见年支
        shensha[i] =
          shensha[i] +
          EightrandomModule.shensha_yearz2earthz(
            this.state.EightDate[1],
            this.state.EightDate[i * 2 + 1],
          ); //年支不见年支
      }
    ...
    var marryinfo = EightrandomModule.getmarryinfo(
      this.state.EightDate,
      this.state.sex,
      r,
      this.state.buildeight,
    );

    var locationself = EightrandomModule.getlocationself(
      curyear,
      this.state.sex == '乾造' ? 0 : 1,
    );
    var house = EightrandomModule.gethouselocation(locationself);
    var yongshen = EightrandomModule.getyongshen(
      this.state.EightDate,
      this.state.buildeight,
      curluckyear[1],
      this.state.precent,
    ...
    //身旺判断
    var ret_powerself = EightrandomModule.getpowerself(
      this.state.EightDate,
      this.state.buildeight,
      curluckyear[1],
      this.state.precent,
    ...
    base.push(
      '喜用: ' +
        yongshen.xishen +
        yongshen.yongshen +
        (yongshen.special != undefined ? '(' + yongshen.special + ')' : ''),
    );
    base.push(' ');
          ...
          <View style={styles.container}>
            <WingBlank size="lg" style={{backgroundColor: '#ffffff'}}>
              <Accordion
                ...
                <Accordion.Panel
                  ...
                  <View>
                        ...
                        <View style={styles.container}>
                          <View style={styles.grid}>
                            <Text
                              style={{
                                fontSize:
                                  FontStyleConfig.getFontApplySize() + 14,
                              }}>
                              {' '}
                              {dataItem}
                            </Text>
                    ...
                    <Grid
                      ...
                      renderItem={dataItem => {
                        if (
                          undefined != dataItem.info &&
                          dataItem.info.length === 3
                        ) {
                          ...
                          return <View style={{height: 90}}>{a}</View>;
                        } else {
                          return (
                            <View style={styles.container}>
                              <View style={styles.grid}>
                                <Text
                                  ...
                                  {this.getColor(
                                    dataItem.info,
                                    FontStyleConfig.getFontApplySize() + 14 + 4,
                                  )}
                                </Text>
                              </View>
                            </View>
                ...
                <Accordion.Panel
                  header={'大运排盘'}
                  styles={{backgroundColor: '#ffffff', color: '#ffffff'}}>
                  <Grid
                    data={years}
                    columnNum={8}
                    hasLine={false}
                    ...
                    onPress={(_el: any, index: any) =>
                      this.changeyear(Number(index % 8), '')
                    }
                ...
                <Accordion.Panel
                  ...
                  <Grid
                    ...
                    onPress={(_el: any, index: any) =>
                      this.changeyear('', Number(_el.split(' ')[1]))
                    }
                    renderItem={(dataItem, itemIndex) =>
                      this.renderminyearItem(dataItem)
                    }
                  />
                </Accordion.Panel>
                <Accordion.Panel
                        ...
                        {marryinfo.map(item => {
                          return (
                            <Text
                              ...
                              {item}
                              {EightrandomModule.tipfire(item)}
                            </Text>
                          );
                        })}
                      </View>
